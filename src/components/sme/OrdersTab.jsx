...
export default function OrdersTab({ bizData }) {
  ...
  const saveField = async (id, field, value) => {
    ...
    if (!error && field === 'status_message') {
      setHighlightMsg(prev => ({ ...prev, [id]: true }));
    }
    fetchOrders(); 
  };

  // --- REFINED WHATSAPP LINK CONSTRUCTION ---
 const sendWA = (order, type) => {
    // 1. Clean the phone number: Remove all non-digits
    const rawPhone = order.customer_whatsapp.replace(/\D/g, '');
    
    // 2. Format for international: 07xxxxxxxx -> 947xxxxxxxx
    const formattedPhone = rawPhone.startsWith('0') 
      ? `94${rawPhone.substring(1)}` 
      : rawPhone;

    // 3. Construct message (Removed '#' to prevent WhatsApp link breaking)
    let msg = "";
    if (type === 'status') {
    ...
    // 4. Update highlights
    if (type === 'status') setHighlightStatus(prev => ({ ...prev, [order.id]: false }));
    else setHighlightMsg(prev => ({ ...prev, [order.id]: false }));
    
    // 5. Use wa.me shortlink for better compatibility
  ...
  if (loading) return <div className="p-10 text-center animate-pulse font-black text-slate-400 text-xs tracking-widest uppercase">Syncing Orders...</div>;
      ...
      <div className="grid gap-6">
        {filteredOrders.map(order => (
          <div key={order.id} className={`bg-white border transition-all rounded-[2.5rem] p-6 md:p-8 shadow-sm hover:shadow-md ${order.is_favourite ? 'border-amber-200 ring-4 ring-amber-500/5' : 'border-slate-100'}`}>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-100/50">
                      <th className="p-4 text-[9px] font-black uppercase text-slate-500 tracking-widest">Item Selection</th>
                      <th className="p-4 text-[9px] font-black uppercase text-slate-500 text-center tracking-widest">Qty</th>
                      <th className="p-4 text-[9px] font-black uppercase text-slate-500 text-right tracking-widest">Line Total</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={i}>
                        <td className="p-4">
                          ...
                          {item.chosenVariation && (
                            <p className="text-[9px] font-bold text-indigo-500 uppercase mt-1">Option: {item.chosenVariation.variation_name}</p>
                          )}
                        </td>
                        <td className="p-4 text-center text-xs font-black text-slate-600 italic">x{item.cartQty}</td>
                        <td className="p-4 text-right text-xs font-black text-slate-800">Rs.{(item.unit_price * item.cartQty).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="bg-white font-black italic text-indigo-600">
                      <td colSpan="2" className="p-4 text-right uppercase text-[10px] tracking-widest">Grand Total</td>
            ...
            {/* CUSTOMER DETAILS & PRIVATE REMARKS */}
            <div className="mt-8 grid lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                >
                  <User size={14} /> {showDetails[order.id] ? 'Hide' : 'Show'} Customer Profile
                </button>
                ...
                {showDetails[order.id] && (
                  <div className="bg-indigo-50/50 p-6 rounded-[2rem] border border-indigo-100 space-y-4 animate-in slide-in-from-top-2">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-[10px] text-indigo-400 uppercase font-black tracking-widest flex items-center gap-1"><Tag size={10}/> Name</p>
                        <p className="text-xs font-black text-slate-800 uppercase">{order.customer_name}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-indigo-400 uppercase font-black tracking-widest flex items-center gap-1"><Phone size={10}/> WhatsApp</p>
                        <p className="text-xs font-black text-slate-800">{order.customer_whatsapp}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-[10px] text-indigo-400 uppercase font-black tracking-widest flex items-center gap-1"><MapPin size={10}/> Delivery Address</p>
                      <p className="text-xs font-bold text-slate-600 leading-relaxed">{order.customer_address}</p>
                    </div>
                    {order.other_contact && (
                      <div>
                        <p className="text-[10px] text-indigo-400 uppercase font-black tracking-widest">Secondary Contact</p>
                    ...
                    <div className="pt-3 border-t border-indigo-100">
                      ...
                      <div className="p-3 bg-white/60 rounded-xl border border-indigo-100">
                        <p className="text-xs font-bold italic text-slate-500">
                          {order.customer_remarks || "No instructions provided."}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
                    <StickyNote size={12}/> Private Admin Remarks
                  </label>
                  <textarea 
              ...
              <div className="space-y-4">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Send WhatsApp Update</label>
                <div className="flex gap-3">
                  <textarea 
                    placeholder="Type custom message to client..."
